...
import { FormEvent, useRef, useState, useEffect } from "react"; // ✅ Added useEffect here
import { Button, Col, Form, Row, Stack } from "react-bootstrap";
import { Link, useNavigate } from "react-router-dom";
import CreatableReactSelect from "react-select/creatable";
import { NoteData, Tag } from "./App";
...
// --- Existing customSelectStyles (unchanged) ---
const customSelectStyles: StylesConfig = {
  // Styles the main input box
  control: (baseStyles, state) => ({
    ...baseStyles,
...
export function NoteForm({
  ...
  // ✅ Added state to track textarea manually (so we can set value dynamically)
  const [pitch, setPitch] = useState(markdown);
  ...
  function handleSubmit(e: FormEvent) {
    ...
    onSubmit({
      title: titleRef.current!.value,
      markdown: pitch, // ✅ use our state instead of direct ref
      tags: selectedTags,
    });
  ...
  // ✅ Added: Function to open your  generator
  const handleOpenGenerator = () => {
    const titleParam = encodeURIComponent(titleRef.current?.value || "");
    const domainParam = encodeURIComponent(
      selectedTags.map((t) => t.label).join(",") || "general"
    );

    window.open(
      `http://localhost:8081/?title=${titleParam}&domain=${domainParam}`,
      "_blank"
    );
  };

  // ✅ Added: Listen for when user returns and auto-fill the pitch
  useEffect(() => {
    const handleFocus = () => {
      const generatedPitch = localStorage.getItem("generatedPitch");
      if (generatedPitch) {
        setPitch(generatedPitch);
        localStorage.removeItem("generatedPitch");
        alert("AI-generated pitch added!");
      }
    };
      ...
      <Form onSubmit={handleSubmit} className="form">
        ...
        <Form.Group controlId="markdown" className="form-group">
          ...
          <Button
            ...
            className="mt-2 uiverse-btn-secondary"
          >
            Generate with AI
          </Button>
        </Form.Group>
